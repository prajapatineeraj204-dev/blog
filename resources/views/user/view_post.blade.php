...
{{-- form comment --}}
            <div class="col-4 mt-3">
                <form action="{{route('user.comment',$data->id)}}" method="post">
                    @csrf
                    <div class="mb-3">
            ...
            {{-- post --}}
                    ...
                    @foreach($comment as $data)
                        <ul>
                            <li>{{$data->created_at->format('d/m/y')}}  {{$data->created_at->format('H:m')}} </li>
                            <p>Comment By::{{$data->user->name}}</p>
                        </ul>
                        <p style="margin-left:120px; ">{{$data->comment}}</p>
                        <hr>
                    @endforeach
